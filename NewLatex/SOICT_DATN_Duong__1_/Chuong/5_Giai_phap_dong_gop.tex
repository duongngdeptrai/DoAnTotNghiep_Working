...
Đề tài lựa chọn module SIM7600 (được giới thiệu chi tiết tại mục \ref{subsection:3.1.2}) — module tích hợp cả bộ thu GNSS (GPS/GLONASS/BeiDou) lẫn modem di động 4G LTE Cat-4 trong một linh kiện duy nhất với một anten duy nhất và một cổng UART duy nhất.
...
Hệ thống triển khai \textbf{hai chế độ geofence} được lưu trong trường \texttt{mode} của mỗi đối tượng geofence trong collection \texttt{geofence\_configs}:

\textbf{a. Chế độ Fixed (vùng hình tròn):} Backend tính khoảng cách từ tọa độ hiện tại $(lat_p, lng_p)$ đến tâm geofence $(lat_c, lng_c)$ bằng công thức Haversine (trình bày tại mục \ref{section:3.5}). Nếu khoảng cách $d > radiusM$, hệ thống kích hoạt cảnh báo. Đây là chế độ phù hợp khi phụ huynh muốn giới hạn trẻ trong một khu vực cụ thể.

\textbf{b. Chế độ Mobile (hành lang đường đi):} Phụ huynh vẽ tuyến đường trên bản đồ trong ứng dụng; tuyến đường được lưu dưới dạng danh sách các điểm tọa độ $\{(lat_1, lng_1), (lat_2, lng_2), \ldots, (lat_n, lng_n)\}$ trong trường \texttt{path} của geofence. Backend tính khoảng cách từ điểm vị trí hiện tại đến \textbf{từng đoạn thẳng} của tuyến đường theo phép chiếu vuông góc:
...
Cả hai chế độ được xử lý trong \textbf{cùng một pipeline backend}: mỗi khi nhận được tọa độ mới từ MQTT, hàm kiểm tra geofence chạy lần lượt qua tất cả geofence đang hoạt động của thiết bị, phân nhánh theo trường \texttt{mode}, tính khoảng cách tương ứng, và lưu kết quả \texttt{insideGeofence} cùng \texttt{distanceFromCenterM} vào bản ghi \texttt{locations}.

\subsection{Kết quả đạt được}

Giải pháp geofencing hai chế độ mang lại tính linh hoạt đáng kể trong thực tế sử dụng. Phụ huynh có thể kết hợp cả hai loại trong cùng một hệ thống: ví dụ đặt vùng hình tròn xung quanh trường học và vùng hành lang dọc theo con đường trẻ thường đi. Thuật toán hoàn toàn tự chủ trên backend, không phụ thuộc dịch vụ bên ngoài, cho phép kiểm soát hoàn toàn logic phát hiện vi phạm và dễ dàng mở rộng thêm chế độ mới trong tương lai.
...
Kiến trúc lai đạt độ trễ end-to-end (từ khi ESP32 đọc tọa độ GPS đến khi bản đồ Flutter cập nhật) dưới 5 giây trong điều kiện mạng bình thường, trong đó phần lớn độ trễ đến từ chu kỳ cập nhật GPS 5 giây của phần cứng. Độ trễ truyền thông thuần túy (MQTT broker → FastAPI → WebSocket → Flutter) đo được dưới 300 mili giây trong điều kiện thử nghiệm. Kiến trúc này cũng cho phép nhiều phụ huynh (nhiều kết nối WebSocket) cùng theo dõi một thiết bị mà không cần thay đổi firmware hay logic MQTT.
...
Khi backend phát hiện vi phạm geofence, hệ thống kích hoạt \textbf{ba kênh thông báo song song}, không phụ thuộc nhau. Khi nhận tín hiệu SOS, hệ thống kích hoạt \textbf{hai kênh} (WebSocket và Telegram):
...
\begin{enumerate}
    ...
    \item \textbf{Telegram Bot:} Backend gọi Telegram Bot API để gửi tin nhắn văn bản kèm tọa độ GPS và liên kết Google Maps trực tiếp đến tài khoản Telegram của phụ huynh. Kênh này hoạt động hoàn toàn độc lập với ứng dụng di động và tốt kể cả khi ứng dụng đang đóng. Với tín hiệu SOS, tin nhắn bao gồm thêm link bản đồ Google Maps định vị tức thì vị trí của trẻ.

    \item \textbf{Email (chỉ với cảnh báo geofence):} Backend gửi email cảnh báo qua SMTP đến địa chỉ email đã đăng ký. Kênh này là lớp dự phòng cuối cùng, đảm bảo phụ huynh có bằng chứng ghi nhận và có thể tra cứu lại lịch sử cảnh báo ngay cả khi không nhận được hai kênh trên.
\end{enumerate}
...
Ba kênh được gọi bất đồng bộ song song (sử dụng \texttt{asyncio.gather}) để giảm thiểu độ trễ tổng thể — thời gian kích hoạt cảnh báo không bị kéo dài do phải chờ lần lượt từng kênh.

\subsection{Kết quả đạt được}

Cơ chế ba kênh song song đảm bảo phụ huynh nhận được cảnh báo trong mọi tình huống sử dụng thực tế: đang xem ứng dụng, đang dùng điện thoại cho việc khác, hay thậm chí không có điện thoại bên cạnh nhưng đang ở máy tính (nhận qua Telegram Web hoặc Email). Trong thử nghiệm thực tế, cả ba kênh đều gửi thành công trong vòng 3 giây sau khi phát hiện vi phạm, với Telegram là kênh nhanh nhất (thường dưới 1 giây) và Email là kênh chậm nhất nhưng đáng tin cậy nhất cho mục đích lưu trữ.
