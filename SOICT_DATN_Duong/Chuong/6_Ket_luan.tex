...
\textbf{Thiết bị IoT hoạt động độc lập} mà không cần điện thoại của trẻ — mục tiêu cốt lõi phân biệt hệ thống này với các giải pháp như Life360. ESP32 kết hợp với SIM7000G đọc tọa độ GPS và truyền qua mạng di động (GPRS/APN Viettel) hoàn toàn tự chủ, với máy trạng thái tự phục hồi khi mất tín hiệu GPS hoặc rớt kết nối mạng.
...
\textbf{Cảnh báo đa kênh} qua WebSocket, Telegram Bot và Email đảm bảo phụ huynh nhận được thông báo kịp thời trong mọi tình huống sử dụng. Tín hiệu SOS khẩn cấp được gửi tức thì kèm liên kết định vị Google Maps.
...
\textbf{Hạn chế còn tồn tại:} Hệ thống chưa có thiết kế PCB tùy chỉnh — phần cứng hiện tại dùng module phát triển ESP32 và bo mạch SIM7000G dạng breakout board, chưa tối ưu về kích thước và tiêu thụ điện cho sản phẩm thương mại. Giao thức xác thực hiện tại dùng Bearer token đơn giản (user\_id) chưa ký số, phù hợp nguyên mẫu nhưng cần nâng cấp lên JWT đầy đủ khi đưa vào sản xuất. Ngoài ra, chức năng chia sẻ thiết bị giữa nhiều phụ huynh (shared permission) đã được thiết kế cơ sở dữ liệu hỗ trợ nhưng giao diện ứng dụng chưa hoàn thiện đầy đủ.

\section{Hướng phát triển}
\label{section:6.2}

\textbf{Thiết kế PCB và đóng gói sản phẩm:} Bước phát triển tự nhiên tiếp theo là thiết kế bo mạch in tùy chỉnh tích hợp ESP32, SIM7000G, pin Li-Po và mạch sạc trong một vỏ đeo nhỏ gọn. PCB tùy chỉnh cho phép tối ưu hóa tiêu thụ điện, giảm kích thước và cải thiện độ bền cơ học cho trẻ em sử dụng hàng ngày.

\textbf{Chế độ ngủ sâu và NB-IoT:} Triển khai deep sleep cho ESP32 và SIM7000G giữa các chu kỳ gửi dữ liệu (wake on timer), kết hợp với chuyển sang chuẩn NB-IoT thay vì GPRS khi có phủ sóng, nhằm kéo dài thời lượng pin từ vài giờ lên vài ngày trên một lần sạc.
...
\textbf{Bảo mật và xác thực nâng cao:} Nâng cấp cơ chế xác thực lên JWT có thời hạn và refresh token, mã hóa kênh MQTT bằng TLS, và triển khai xác thực hai yếu tố (2FA) cho ứng dụng phụ huynh nhằm đảm bảo an toàn dữ liệu vị trí trẻ em trong môi trường sản xuất thực tế.
