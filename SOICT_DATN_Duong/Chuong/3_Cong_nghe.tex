...
ESP32 là vi điều khiển 32-bit do Espressif Systems phát triển, sử dụng lõi Xtensa LX6 dual-core với xung nhịp tối đa 240\,MHz \cite{espressif_esp32}. Chip tích hợp sẵn Wi-Fi 802.11 b/g/n, Bluetooth 4.2 và BLE, cùng 520\,KB SRAM và hỗ trợ flash ngoài lên đến 16\,MB. Ngoài ra, ESP32 cung cấp đầy đủ giao tiếp ngoại vi như UART, SPI, I\textsuperscript{2}C và ADC, đáp ứng yêu cầu giao tiếp với module SIM7000G qua UART.

Đặc điểm nổi bật nhất của ESP32 đối với thiết kế này là các chế độ tiết kiệm năng lượng linh hoạt. Ở chế độ deep sleep, vi điều khiển chỉ tiêu thụ khoảng 10\,µA, cho phép kéo dài đáng kể thời lượng pin trong các khoảng thời gian không cần xử lý giữa hai chu kỳ gửi dữ liệu GPS. Kết hợp với timer wake-up nội bộ, ESP32 có thể định kỳ thức dậy, đọc GPS, gửi MQTT rồi ngủ lại theo chu kỳ đã cấu hình.
...
SIM7000G là module đa băng tần do SIMCom Wireless Solutions sản xuất, tích hợp đồng thời bộ thu GPS (GNSS: GPS/GLONASS/BeiDou) và modem di động hỗ trợ NB-IoT, LTE-M Cat-M1 và GPRS trong một linh kiện duy nhất \cite{simcom_sim7000g}. Module giao tiếp với vi điều khiển chủ thông qua giao diện UART sử dụng tập lệnh AT command tiêu chuẩn, cho phép ESP32 điều khiển cả hai chức năng định vị và kết nối dữ liệu qua một kênh giao tiếp duy nhất.

Về hiệu năng định vị, SIM7000G đạt độ chính xác GPS khoảng 2.5\,m CEP (Circular Error Probable) trong điều kiện lý tưởng ngoài trời với khoảng 8 vệ tinh. Thời gian khởi động lạnh (cold start) khoảng 30 giây, trong khi khởi động nóng (hot start) chỉ khoảng 1 giây khi dữ liệu vệ tinh đã được cache. Dữ liệu vị trí được cung cấp ở định dạng NMEA 0183, từ đó firmware trích xuất chuỗi \texttt{\$GPGGA} để lấy tọa độ latitude, longitude và độ cao.

Các lựa chọn thay thế gồm: ghép module GPS u-blox NEO-6M với modem SIM800L rời (hai module riêng biệt tăng độ phức tạp phần cứng, số lượng linh kiện và diện tích bo mạch), module SIM808 (hỗ trợ GPS và GPRS nhưng không có NB-IoT/LTE-M, băng thông thấp hơn), và GPS tracker module tích hợp sẵn như Quectel MC60 (tương tự nhưng ít tài liệu hỗ trợ hơn trong cộng đồng Việt Nam). SIM7000G được lựa chọn vì tích hợp GPS và kết nối di động thế hệ mới trong một module duy nhất, giảm thiểu số lượng linh kiện, đơn giản hóa firmware, và có khả năng sử dụng mạng NB-IoT tiết kiệm điện khi có phủ sóng phù hợp.
...
Kiến trúc MQTT dựa trên ba thành phần: \textit{publisher} (thiết bị IoT gửi dữ liệu), \textit{broker} (máy chủ trung gian nhận và phân phối bản tin), và \textit{subscriber} (backend đăng ký nhận bản tin). Thiết bị gửi tọa độ GPS lên topic \texttt{gps/\{device\_id\}/location}; backend đăng ký topic này và xử lý ngay khi nhận được bản tin. Mô hình này tách biệt hoàn toàn thiết bị và backend, cho phép nhiều subscriber cùng nhận dữ liệu từ một thiết bị mà không cần kết nối trực tiếp.
...
Trong hệ thống này, cả thiết bị IoT (ESP32) lẫn backend (FastAPI) đều kết nối đến \texttt{broker.emqx.io} qua cổng 1883. Thiết bị publish tọa độ GPS lên topic \texttt{dotn/duong29/gps/child\_01} và tín hiệu SOS lên \texttt{dotn/duong29/sos/child\_01}; backend đăng ký nhận bằng wildcard \texttt{dotn/duong29/gps/\#} và \texttt{dotn/duong29/sos/\#}. Việc sử dụng namespace topic riêng (\texttt{dotn/duong29/}) giúp tránh xung đột với các client khác trên cùng broker công khai.
...
Cấu trúc collection chính bao gồm: \texttt{users} (thông tin tài khoản phụ huynh), \texttt{devices} (thông tin thiết bị GPS), \texttt{locations} (tọa độ GPS theo chuỗi thời gian), và \texttt{geofences} (thông tin vùng an toàn). Dịch vụ MongoDB Atlas cung cấp gói miễn phí 512\,MB đủ cho giai đoạn thử nghiệm, kèm theo sao lưu tự động và giao diện web quản lý.
...
Hệ thống xác thực người dùng theo cơ chế Bearer token kết hợp với bcrypt. Khi đăng ký, mật khẩu được băm bằng thuật toán bcrypt (với salt tự động) trước khi lưu vào cơ sở dữ liệu, đảm bảo không lưu trữ mật khẩu dưới dạng plaintext. Khi đăng nhập, backend xác minh mật khẩu bằng cách so sánh giá trị băm; nếu hợp lệ, server trả về user\_id dưới dạng Bearer token. Ứng dụng di động đính kèm token này vào header \texttt{Authorization: Bearer \{user\_id\}} của mọi yêu cầu API tiếp theo; backend trích xuất và dùng trực tiếp để định danh người dùng mà không cần tra cứu session.

Cơ chế này đơn giản, stateless và phù hợp với quy mô nguyên mẫu. So với JWT, cách tiếp cận này bỏ qua việc ký số và kiểm tra thời hạn token, song đủ đáp ứng yêu cầu bảo vệ tài nguyên API trong môi trường phát triển và thử nghiệm.
