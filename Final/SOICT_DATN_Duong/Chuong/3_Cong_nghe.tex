...
SIM7600 là module đa băng tần do SIMCom Wireless Solutions sản xuất, tích hợp đồng thời bộ thu GPS (GNSS: GPS/GLONASS/BeiDou) và modem di động hỗ trợ 4G LTE Cat-1 cùng khả năng fallback về 3G/2G trong một linh kiện duy nhất \cite{simcom_sim7600}. Module giao tiếp với vi điều khiển chủ thông qua giao diện UART sử dụng tập lệnh AT command tiêu chuẩn, cho phép ESP32 điều khiển cả hai chức năng định vị và kết nối dữ liệu qua một kênh giao tiếp duy nhất.

Về hiệu năng định vị, SIM7600 đạt độ chính xác GPS khoảng 2{,}5\,m CEP (Circular Error Probable) trong điều kiện lý tưởng ngoài trời với khoảng 8 vệ tinh. Thời gian khởi động lạnh (cold start) khoảng 30 giây, trong khi khởi động nóng (hot start) chỉ khoảng 1 giây khi dữ liệu vệ tinh đã được cache. So với các module thế hệ trước sử dụng GPRS (tốc độ tối đa ~114\,kbps), SIM7600 với 4G LTE Cat-1 cung cấp băng thông lên đến 10\,Mbps DL, đảm bảo bản tin MQTT được truyền với độ trễ tối thiểu ngay cả khi tải mạng cao. Firmware giao tiếp với SIM7600 thông qua thư viện TinyGSM — thư viện này trừu tượng hóa toàn bộ tập lệnh AT command, cung cấp API cấp cao như \texttt{modem.getGPS()} để lấy tọa độ và \texttt{modem.gprsConnect()} để kết nối mạng di động, giúp firmware ngắn gọn và không phụ thuộc vào cú pháp AT command cụ thể của từng module.

Các lựa chọn thay thế gồm: ghép module GPS u-blox NEO-6M với modem SIM800L rời (hai module riêng biệt tăng độ phức tạp phần cứng, số lượng linh kiện và diện tích bo mạch), module SIM808 (hỗ trợ GPS và GPRS nhưng chỉ kết nối 2G, băng thông thấp), và module SIM7000G (hỗ trợ NB-IoT/LTE-M tiết kiệm điện hơn nhưng băng thông thấp hơn và phủ sóng NB-IoT tại Việt Nam còn hạn chế). SIM7600 được lựa chọn vì tích hợp GPS và kết nối 4G LTE trong một module duy nhất, giảm thiểu số lượng linh kiện, đơn giản hóa firmware, và đảm bảo kết nối ổn định trên toàn bộ vùng phủ sóng 4G Viettel.
